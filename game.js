...
/* ============================================================
   GLOBAL TOP-LEVEL INSPECTION MODAL HANDLERS
   ============================================================ */
const SAMAY_ARTIFACT_DETAILS = {
  price: {
...
window.samayOpenInspect = function(docId) {
  console.log("[SAMAY] openInspect triggered for:", docId);
  const overlay = document.getElementById("artifact-inspect-overlay");
  const content = document.getElementById("inspect-content-body");
  const slip = document.getElementById("approval-stamp-slip");

  if (!overlay || !content) {
    console.error("[SAMAY] Missing overlay or content element!");
    return;
  }

  const data = SAMAY_ARTIFACT_DETAILS[docId];
  if (!data) return;
  ...
  overlay.style.cssText = "display: flex !important; position: fixed !important; inset: 0 !important; z-index: 99999 !important; align-items: center !important; justify-content: center !important;";

  if (slip) slip.style.display = "block";
};

window.samayCloseInspect = function() {
  const overlay = document.getElementById("artifact-inspect-overlay");
  if (overlay) {
...
// Global click event listener at top-level script evaluation time
document.addEventListener("click", (e) => {
  if (e.target.closest("#btn-close-inspect, #inspect-backdrop")) {
    window.samayCloseInspect();
    return;
  }
  ...
  if (trigger) {
    ...
    if (docTarget && SAMAY_ARTIFACT_DETAILS[docTarget]) {
      window.samayOpenInspect(docTarget);
    }
  }
});
